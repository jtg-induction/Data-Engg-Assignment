...
SELECT
    DISTINCT 'JOSH_CLEAN_AUTOMOBILES' AS nation,
    date_trunc('month', calendardate) :: date AS from_period,
    (
        date_trunc('month', calendardate) + INTERVAL '1 month' - INTERVAL '1 day'
    ) :: date AS to_period,
    1 AS compute_period,
